// Author : iflytek
// Modified by ziqi han
// Latest Modify : 04 / 27 / 2023
// Description : Communication between chassis and upper computer
...
namespace ucarController
{
  ...
  {
    if (current_battery_percent_ == -1) // The initial value is -1. That is, no power is obtained
    {
    ...
    {
      res.battery_state.power_supply_status = 2;     // Discharging is running
      res.battery_state.power_supply_health = 1;     // Good
      res.battery_state.power_supply_technology = 0; // UNKNOWN
      res.battery_state.present = true;              // battery present
      boost::unique_lock<boost::recursive_mutex> lock(Control_mutex_);
      res.battery_state.percentage = current_battery_percent_; // battery percentage
      lock.unlock();
      return true;
    }
  }
  ...
  void baseBringup::processBattery()
  {
    ...
    battery_msg.percentage = current_battery_percent_;
