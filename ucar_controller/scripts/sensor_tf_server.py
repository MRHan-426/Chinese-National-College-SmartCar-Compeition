...
# Description: Publish the TF of sensors coordinate system
        ...
        while not rospy.is_shutdown():
            ...
            self.tf_br.sendTransform((self.camera_pose_x, self.camera_pose_y, self.camera_pose_z),
                                     tf.transformations.quaternion_from_euler(
                                         self.camera_euler_r, self.camera_euler_p, self.camera_euler_y),
                                     rospy.Time.now(),
                                     self.camera_frame,
                                     self.base_frame)
            # send lidar tf
            ...
            self.tf_br.sendTransform((self.imu_pose_x, self.imu_pose_y, self.imu_pose_z),
                                     tf.transformations.quaternion_from_euler(
                                         self.imu_euler_r, self.imu_euler_p, self.imu_euler_y),
                                     rospy.Time.now(),
                                     self.imu_frame,
                                     self.base_frame)
            lock.release()
