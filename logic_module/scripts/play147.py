...
def goal_callback(msg):
    ...
    if msg.status.status == 3:
        if pos_flag == 1:
            rospy.sleep(0.5)
            pos_flag += 2   # 跳过pos_flag = 2,第二个人物识别点
        elif pos_flag == 4:
            rospy.sleep(0.5)
            pos_flag += 2   # 跳过pos_flag = 5， 第三个人物识别点
        elif pos_flag == 7 or pos_flag == 2:
            pos_flag += 1
        else:
            rospy.sleep(0.5)
            pos_flag += 1
...
# -------------------到达临时位置点---------------------------------------
    elif pos_flag == 3 and not reached_temp_flag:
        # 前往二维码
