		...
		if (businessMsg.msgId == 0x01)
		{
			...
			if (major_mic_id <= 5 && major_mic_id >= 0 && mic_angle <= 360 && mic_angle >= 0)
			{
				...
				if (ret1 == 0 && ret2 == 0)
				{
					...
					std_msgs::Int8 startmsg;
	                		startmsg.data = 1;
	                		start_info_pub.publish(startmsg);
...
int main(int argc, char *argv[])
{
	...
	ndHandle.param("appid", appid, std::string("d43172e7"));//appid，需要更换为自己的
	...
	start_info_pub = ndHandle.advertise<std_msgs::Int8>("/start_others",true, 10);
